...
void loop() {
  ...
  int location_percentage =
      map(x1, 0, pixy.frameWidth, 0, 100); // 0 = left 50 = center 100 = right

  for (int i = 0; i < 20; i++) {
    if (i == 0 || i == 20 - 1) {
      printf("|");
    } else if (i == (int)(x0 * 20) / pixy.frameWidth) {
      printf("^");
    } else if (i == 9 || i == 10) {
      printf("|");
    } else {
  ...
  int thresholdlow = 5;
  int motor_min_speed = 0.3;

  bool doDrive = !timer_running || (timer_running && main_timer.read_ms() < 1500);

  //   motor_right.speed(location_percentage < 50 - threshold ? 1 : 0);
  //   motor_left.speed(location_percentage > 50 + threshold ? 1 : 0);

    //   motor_left.speed(doDrive
    //                        ? (location_percentage < 50 - thresholdlow ? (location_percentage < 50 - thresholdhigh ? 0 : 0.5) : 1)
    //                        : 0);
    //   motor_right.speed(doDrive
    //                         ? (location_percentage > 50 + thresholdhigh ? (location_percentage > 50 + thresholdhigh ? 0 : 0.5) : 1)
    //                         : 0);
    ...
    printf("\t %d\t%d\t(%.2f, %.2f)", location_percentage, location_percentage, power_left, power_right);
    printf("\n");
}