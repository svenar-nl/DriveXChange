...
void VL53L1X::startMeasurement(uint8_t offset) {
  ...
  while (leftToSend > 0) {
    ...
    uint8_t toSend =
        30; // Max I2C buffer on Arduino is 32, and we need 2 bytes for address
...
uint16_t VL53L1X::getDistance() {
  return (
      readRegister16(VL53L1_RESULT__FINAL_CROSSTALK_CORRECTED_RANGE_MM_SD0));
}

uint16_t VL53L1X::getSignalRate() {
  // From vl53l1_api.c line 2041
  uint16_t reading = readRegister16(
      VL53L1_RESULT__PEAK_SIGNAL_COUNT_RATE_CROSSTALK_CORRECTED_MCPS_SD0); // <<
                                                                           // 9;
                                                                           // //FIXPOINT97TOFIXPOINT1616
...
#define VL53L1_RANGESTATUS_NONE 255 /*!<No Update. */
  ...
  uint8_t measurementStatus =
      (readRegister(VL53L1_RESULT__RANGE_STATUS) & 0x1F);
